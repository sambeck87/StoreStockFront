...
import type { Item, Branch, PaginationMeta } from '../../types';
import { ArrowLeft, Package, Trash2, Power, Save, Plus, Minus, Pencil } from 'lucide-react';
import { Skeleton, SkeletonTable, EmptyState, Pagination } from '../../components/common';
...
export function CategoryItemsPage() {
  ...
  useEffect(() => { fetchItems(1); }, [id, filters, perPage, branchesLoaded, refreshKey]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
          <select value={filters.branch_id} onChange={(e) => handleFilterChange('branch_id', e.target.value)}
            className="flex-1 min-w-[120px] px-2.5 py-1.5 text-xs border border-[var(--color-border)] dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-[var(--color-accent)]/30 focus:border-[var(--color-accent)] outline-none">
        ...
        <>
          ...
          <div className="lg:hidden space-y-4">
            {items.map((item) => {
              ...
              const isEmpty = item.current_quantity === undefined || item.current_quantity === null || item.current_quantity <= 0;
              const stockClass = isEmpty ? '!border-red-300 dark:!border-red-700 !bg-red-50/60 dark:!bg-red-900/20' : isLowStock ? '!border-amber-300 dark:!border-amber-700 !bg-amber-50/60 dark:!bg-amber-900/20' : '';
              return (
                <Card key={item.id} className={`${stockClass}`}>
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex-1">
                      <input value={drafts[item.id]?.name ?? item.name} onChange={(e) => handleDraftChange(item.id, 'name', e.target.value)}
                        className="w-full font-semibold text-gray-900 dark:text-gray-100 bg-transparent border-b border-transparent hover:border-gray-300 focus:border-[var(--color-accent)] focus:outline-none transition-colors" />
                      <div className="flex items-center gap-2 mt-1">
                        <input value={drafts[item.id]?.measure ?? item.measure ?? ''} onChange={(e) => handleDraftChange(item.id, 'measure', e.target.value)}
                          className="text-sm text-gray-500 dark:text-gray-400 bg-transparent border-b border-transparent hover:border-gray-300 focus:border-[var(--color-accent)] focus:outline-none transition-colors w-24" placeholder="Unidad" />
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <div className="flex items-center gap-0.5 group mt-1">
                        <button onClick={() => { const c = Number(drafts[item.id]?.current_quantity ?? item.current_quantity) || 0; handleDraftChange(item.id, 'current_quantity', String(Math.max(0, c - 1))); }}
                          className="w-5 h-5 flex items-center justify-center rounded-full bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-400 transition-all focus:outline-none">
                          <Minus className="w-3 h-3" />
                        </button>
                        <input type="number" value={drafts[item.id]?.current_quantity ?? item.current_quantity ?? ''} onChange={(e) => handleDraftChange(item.id, 'current_quantity', e.target.value)}
                          className={`w-14 text-center py-0.5 bg-transparent border-b border-transparent hover:border-gray-300 focus:border-[var(--color-accent)] focus:outline-none transition-colors text-sm font-semibold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${isEmpty ? 'text-red-600 dark:text-red-400' : isLowStock ? 'text-amber-600 dark:text-amber-400' : 'text-gray-900 dark:text-gray-100'}`} />
                        <button onClick={() => { const c = Number(drafts[item.id]?.current_quantity ?? item.current_quantity) || 0; handleDraftChange(item.id, 'current_quantity', String(c + 1)); }}
                          className="w-5 h-5 flex items-center justify-center rounded-full bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-400 transition-all focus:outline-none">
                          <Plus className="w-3 h-3" />
                        </button>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-0.5 group mt-1">
                        <button onClick={() => { const c = Number(drafts[item.id]?.minimum_quantity ?? item.minimum_quantity) || 0; handleDraftChange(item.id, 'minimum_quantity', String(Math.max(0, c - 1))); }}
                          className="w-5 h-5 flex items-center justify-center rounded-full bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-400 transition-all focus:outline-none">
                          <Minus className="w-3 h-3" />
                        </button>
                        <input type="number" value={drafts[item.id]?.minimum_quantity ?? item.minimum_quantity ?? ''} onChange={(e) => handleDraftChange(item.id, 'minimum_quantity', e.target.value)}
                          className="w-14 text-center py-0.5 bg-transparent border-b border-transparent hover:border-gray-300 focus:border-[var(--color-accent)] focus:outline-none text-gray-900 dark:text-gray-100 transition-colors text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
                        <button onClick={() => { const c = Number(drafts[item.id]?.minimum_quantity ?? item.minimum_quantity) || 0; handleDraftChange(item.id, 'minimum_quantity', String(Math.max(0, c + 1))); }}
                          className="w-5 h-5 flex items-center justify-center rounded-full bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-400 transition-all focus:outline-none">
                          <Plus className="w-3 h-3" />
                        </button>
                    ...
                    <div>
                      ...
                      <div className="flex items-center mt-1">
                        <span className="text-gray-400 text-xs mr-0.5">$</span>
                        <input type="number" step="0.01" value={drafts[item.id]?.cost !== undefined ? drafts[item.id]?.cost : (item.cost !== undefined && item.cost !== null ? Number(item.cost).toFixed(2) : '')} onChange={(e) => handleDraftChange(item.id, 'cost', e.target.value)}
                          className="w-16 px-1 py-0.5 bg-transparent border-b border-transparent hover:border-gray-300 focus:border-[var(--color-accent)] focus:outline-none text-gray-600 dark:text-gray-400 transition-colors text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 mt-4 pt-3 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => saveInline(item)} disabled={!hasChanges(item.id)}
                      className={hasChanges(item.id) ? 'text-[var(--color-accent)]' : 'text-gray-300'}>
